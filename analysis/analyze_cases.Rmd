...
Load word count and dictionary data (undo comment to do robustness check):

```{r load data, include=TRUE}
wordcount <- read_csv(here("data", "foranalyses", "wordcount.csv"), show_col_types = FALSE)
#wordcount <- read_csv(here("data", "foranalyses", "wordcount_robust.csv"), show_col_types = FALSE)
...
```{r logistic regression, include=TRUE, eval = FALSE}
...
weights_lr <- weights_lr_raw %>% 
  ...
  arrange(desc(zeta)) %>%
  unique() %>%
  write_csv(here("output", "results", "cases_lr_lang.csv"))
  #write_csv(here("output", "results", "cases_lr_lang_robust.csv"))

weights_lr_fam <- bind_weights_fam(d_wide_small) %>% 
  arrange(desc(zeta)) %>%
  write_csv(here("output", "results", "cases_lr_fam.csv"))
  #write_csv(here("output", "results", "cases_lr_fam_robust.csv"))
...
```{r run glmmtmb function, include=TRUE, eval=FALSE}
...
```{r run glmer, include=TRUE, eval=FALSE}

glmer_tmp <- map_dfr(all_groups, ~ run_glmmTMB(.x, dstats, "avgmonth_tmp") )  %>% 
  arrange(coefficient) %>%
  #write_csv(here("output", "results", "glmer_stats.csv"))
  write_csv(here("output", "results", "glmer_stats_robust.csv"))

glmer_pre <- map_dfr(all_groups, ~ run_glmmTMB(.x, dstats, "maxmonth_pre") )  %>% 
  arrange(coefficient) %>%
  #write_csv(here("output", "results", "glmer_stats.csv"), append=TRUE)
  write_csv(here("output", "results", "glmer_stats_robust.csv"), append=TRUE)
...
glmer_tmp <- map_dfr(all_groups, ~ run_glmmTMB(.x, dstats, "avgmonth_tmp") )  %>% 
  arrange(coefficient) %>%
  #write_csv(here("output", "results", "glmer_stats_dict.csv"))
  write_csv(here("output", "results", "glmer_stats_dict_robust.csv"))

glmer_pre <- map_dfr(all_groups, ~ run_glmmTMB(.x, dstats, "maxmonth_pre") )  %>% 
  arrange(coefficient) %>%
  #write_csv(here("output", "results", "glmer_stats_dict.csv"), append=TRUE)
  write_csv(here("output", "results", "glmer_stats_dict_robust.csv"), append=TRUE)
...
Run Bayesian analyses without area as a random effect:
...
run_bayes <- function(groupname, dstats, predictorname) {
  ...
  mdl <- quiet_brm(formula, data = dstats_this, family = "binomial",
                   #iter = 3000, chains = 2, control = list(adapt_delta = 0.95, stepsize = 0.01, max_treedepth = 15)) # used for the full set
                   warmup = 2000, iter = 10000, chains = 2, control = list(adapt_delta = 0.99, stepsize = 0.001, max_treedepth = 20)) # used for the robustness set
  m <- mdl$result
...
bayes_tmp <- map_dfr(all_groups, ~ run_bayes(.x, dstats, "avgmonth_tmp") )  %>% 
  #write_csv(here("output", "results", "bayes_stats_dict.csv"))
  write_csv(here("output", "results", "bayes_stats_dict_robust.csv"))

bayes_pre <- map_dfr(all_groups, ~ run_bayes(.x, dstats, "maxmonth_pre") )  %>% 
  #write_csv(here("output", "results", "bayes_stats_dict.csv"), append=TRUE)
  write_csv(here("output", "results", "bayes_stats_dict_robust.csv"), append=TRUE)
...
run_bayes_area <- function(groupname, dstats, predictorname) {
  ...
  mdl <- quiet_brm(formula, data = dstats_this, family = "binomial",
                   warmup = 2000, iter = 10000, chains = 2, control = list(adapt_delta = 0.99, stepsize = 0.001, max_treedepth = 20)) 
  m <- mdl$result
...
# there are 6 cases with 1-2 divergent transitions
expect_equal(nrow(subset(bayes %>% filter(!is.na(warnings)))), 6)
# there are 18 cases with 1-13 divergent transitions, we only report those with less than 5 divergent transitions
expect_equal(nrow(subset(bayes_r_raw %>% filter(!is.na(warnings)))), 18)
...
bayes_r <- bayes_r_raw %>%
  mutate(model = paste(group, "by", predictor),
         coefficient = case_when(
           model == "wind_group by maxmonth_pre" ~ NA,
           model == "wind_group by maxmonth_wnd" ~ NA,
           model == "smell_group by maxmonth_wnd" ~ NA,
           TRUE ~ coefficient
         ),
         lower_ci = case_when(
           model == "wind_group by maxmonth_pre" ~ NA,
...
# there was a converge issue for counts for ice predicted by windspeed, so we'll use simpler model without dictionary as a random effect
pvals_nodict_rob <- read_csv(here("output", "results", "bottomup_stats_area_robust.csv"), show_col_types = FALSE) %>%
  filter(word == "ice", predictor == "maxmonth_wnd")

pvals_area_rob <- pvals_area_rob %>%
  bind_rows(pvals_nodict_rob) %>%
  mutate(lnpval=-log(p_val),
         coefficient=round(coefficient, 4),
         p_val=round(p_val, 4)) %>%
  group_by(predictor) %>%
