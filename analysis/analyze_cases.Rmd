...
This notebook contains analysis of case studies (see the main text section called "Seven case studies of lexical elaboration"). It generates Figure 2 in the main text, and Table S3, Table S4, and Figure S7 in the Supporting Material. Note that it takes several hours to generate results for Bayesian analyses.
...
Run the simple model without dictionary as a random effect:
...
Run the full model with dictionary as a random effect:

```{r run glmer, include=TRUE}

glmer_tmp <- map_dfr(all_groups, ~ run_glmmTMB_withdict(.x, dstats, "avgmonth_tmp") )  %>% 
  arrange(coefficient) %>%
  write_csv(here("output", "results", output_glmm_dict))

glmer_pre <- map_dfr(all_groups, ~ run_glmmTMB_withdict(.x, dstats, "maxmonth_pre") )  %>% 
  arrange(coefficient) %>%
  write_csv(here("output", "results", output_glmm_dict), append=TRUE)

glmer_wnd <- map_dfr(all_groups, ~ run_glmmTMB_withdict(.x, dstats, "maxmonth_wnd") )  %>% 
  arrange(coefficient)  %>%
  write_csv(here("output", "results", output_glmm_dict), append=TRUE)

glmer_sub <- map_dfr(all_groups, ~ run_glmmTMB_withdict(.x, dstats, "subsistence") )  %>% 
  arrange(coefficient)  %>%
  write_csv(here("output", "results", output_glmm_dict), append=TRUE)

glmer_pop <- map_dfr(all_groups, ~ run_glmmTMB_withdict(.x, dstats, "logpop") )  %>% 
  arrange(coefficient) %>%
  mutate(predictor="population")  %>%
  write_csv(here("output", "results", output_glmm_dict), append=TRUE)
```
...
```{r run bayes without area, include=TRUE, eval=FALSE}
...
```{r re-run with more iterations, include=TRUE, eval=FALSE}
...
bayes_add <- withlowess %>%
  mutate(predictor = ifelse(predictor == "population", "logpop", predictor)) %>%
  rowwise() %>%
...
bayes_add %>%
  mutate(predictor = ifelse(predictor == "logpop", "population", predictor)) %>%
  write_csv(here("output", "results", output_bayes_dict), append=TRUE)
...
bayes_sub <- map_dfr(all_groups, ~ run_bayes_area_simple(.x, dstats, "subsistence") )  %>% 
  write_csv(here("output", "results", "bayes_stats_dictarea.csv"), append=TRUE)

all_group <- c("wind_group")

bayes_pop <- map_dfr(all_groups, ~ run_bayes_area_simple(.x, dstats, "logpop") )  %>% 
  mutate(predictor="population") %>%
  write_csv(here("output", "results", "bayes_stats_dictarea.csv"), append=TRUE)
...
bayes_tmp <- map_dfr(all_groups, ~ run_bayes_area_simple(.x, dstats, "avgmonth_tmp") )  %>% 
  write_csv(here("output", "results", "bayes_stats_dictarea_robust.csv"))

bayes_pre <- map_dfr(all_groups, ~ run_bayes_area_simple(.x, dstats, "maxmonth_pre") )  %>% 
  write_csv(here("output", "results", "bayes_stats_dictarea_robust.csv"), append=TRUE)

bayes_wnd <- map_dfr(all_groups, ~ run_bayes_area_simple(.x, dstats, "maxmonth_wnd") )  %>% 
  write_csv(here("output", "results", "bayes_stats_dictarea_robust.csv"), append=TRUE)
...
plot_map <- function(word) {
  ...
  map_plot <- ggplot() +
    ...
    geom_point(data = posns, aes(longitude, latitude), color = "grey30", size = 0.7, shape = 16) +
...
Make table on top languages, families, and terms.
...
# We find the top 11 terms most strongly associated with seven concepts in a way that they appear both in the full and robustness set.
...
docterm <- d_long2 %>% select(dict, word, count, dictsize_data) %>% 
  filter(count > 0)
...
create_table_plot <- function(concept, top_five_langs, top_five_fams, top_terms) {

  top_languages <- top_five_langs[top_five_langs$word == paste0(concept, "_group"), "langname"] %>% pull()
  top_families <- top_five_fams[top_five_fams$word == paste0(concept, "_group"), "family"] %>% pull()
  
  table_data <- data.frame(
    Column1 = c("Top languages", top_languages, "Top families", top_families),
    Column2 = c("Top terms", top_terms, rep("", max(0, 10 - length(top_terms))))
  )
  
  table_data$Row <- seq_len(nrow(table_data))
  
  header1_data <- table_data[table_data$Column1 %in% c("Top languages", "Top families"), ]
  header2_data <- table_data[table_data$Column2 == "Top terms", ]
  
  bold_list <- c("Top languages", "Top families", 
                 "Eastern Canadian Inuktitut", "Lakota", "Navajo", "Plains Cree", "Dakota",
                 "German", "Central Siberian Yupik", "Russian",
                 "Hawaiian", "Nyanja", "Scots",
                 "Maori", "Tuamotuan", "Fijian", "Yámana",
                 "Sanskrit", "Zulu", "Marshallese", "Kannada", "Gilbertese",
                 "Eastern Ojibwa", "Chippewa",
                 "Seneca", "K'iche'", "Tangga")
  
  italic_list <- c("Ngalkbun", "Latin", "Mudburra", "Central Tarahumara")
  
  table_data$fontface <- ifelse(
    table_data$Column1 %in% bold_list, 
    "bold", 
    "plain"
  )
  table_data$fontface <- ifelse(
    table_data$Column1 %in% italic_list, 
    "italic", 
    table_data$fontface
  )
  
  p <- ggplot(table_data, aes(x = 1, y = -Row)) +
  # Column 1 with background for headers
  geom_rect(
    data = header1_data,
    aes(xmin = 0.6, xmax = 1, ymin = -Row - 0.5, ymax = -Row + 0.5),
    fill = "gray50", inherit.aes = FALSE
  ) +
  geom_text(aes(x = 0.8, label = table_data$Column1, fontface = fontface), 
            size = ifelse(table_data$Column1 %in% c("Top languages", "Top families"), 3, 2.9), 
            hjust = 0.5, 
            color = ifelse(table_data$Column1 %in% c("Top languages", "Top families"), "white", "black")) +
  geom_rect(
    data = header2_data,
    aes(xmin = 1.01, xmax = 1.3, ymin = -Row - 0.5, ymax = -Row + 0.5),
    fill = "gray50", inherit.aes = FALSE
  ) +
  geom_text(aes(x = 1.15, label = table_data$Column2), 
            size = ifelse(table_data$Column2 == "Top terms", 3, 2.9), 
            fontface = ifelse(table_data$Column2 == "Top terms", "bold", "plain"),
            hjust = 0.5, color = ifelse(table_data$Column2 == "Top terms", "white", "black")) +
  theme_void() +
...
top_snow <- create_table_plot("snow", top_five_langs, top_five_fams, snowcommon)
top_ice <- create_table_plot("ice", top_five_langs, top_five_fams, icecommon)
top_rain <- create_table_plot("rain", top_five_langs, top_five_fams, raincommon)
top_wind <- create_table_plot("wind", top_five_langs, top_five_fams, windcommon)
top_smell <- create_table_plot("smell", top_five_langs, top_five_fams, smellcommon)
top_taste <- create_table_plot("taste", top_five_langs, top_five_fams, tastecommon)
top_dance <- create_table_plot("dance", top_five_langs, top_five_fams, dancecommon)
...
create_env_plot <- function(groupname, variablename, significance, plotd){
  climateplot <- plotd %>%
    ...
    geom_point(size=0.001, color = "gray20") +
...
plot_taste_env <- create_env_plot("taste_group", "temperature (°C)", "none", plotd)
...
create_sub_plot <- function(groupname, significance, plotd){
  subsistenceplot <- plotd %>%
    ...
    geom_jitter(size=0.0001, color = "gray20") +
    geom_boxplot(outlier.size = 0.0001, color = "gray20") +
    labs(y="", x="") +
    mytheme +
...
# no convergence issue is identified for the robustness set
withconvergence <- glmm_r %>% filter(!is.na(warnings_m1_a) | !is.na(warnings_m2_a))
expect_equal(nrow(withconvergence), 0)
...
bayes_r <- read_csv(here("output", "results", "bayes_stats_dict_robust.csv"), show_col_types = FALSE) %>%
  # filter out cases with low ESS
  filter(is.na(warnings) | !startsWith(warnings, "Bulk Effective Samples Size (ESS) is too low"))

# no divergent transitions for the full set
expect_equal(nrow(subset(bayes %>% filter(!is.na(warnings)))), 0)
# no divergent transitions for the robustness set
expect_equal(nrow(subset(bayes_r %>% filter(!is.na(warnings)))), 0)

bayes <- bayes %>%
  mutate(lower_ci = sprintf("%.2f", lower_ci),
...
all_noa <- glmm %>%
  ...
  mutate(p_val_noarea = sprintf("%.2f", p_val_noarea),
...
# 1 convergence issue identified for the full set
withconvergence <- glmm %>% filter(!is.na(warnings_m1) | !is.na(warnings_m2))
expect_equal(nrow(glmm %>% filter(!is.na(warnings_m1) | !is.na(warnings_m2))), 1)

# we'll use results from the analysis without a dictionary as a random effect for these cases
glmm_nodict <- read_csv(here("output", "results", "glmer_stats.csv"), show_col_types = FALSE) %>%
  mutate(case=paste(group, "by", predictor))

convergence_case <- withconvergence$case

replacement_row <- glmm_nodict %>%
...
# 7 convergence issues identified for the robustness set
withconvergence <- glmm_r %>% filter(!is.na(warnings_m1) | !is.na(warnings_m2))
expect_equal(nrow(withconvergence), 7)
...
# there are 9 cases with divergent transitions
expect_equal(nrow(subset(bayes %>% filter(!is.na(warnings)))), 9)
# there are 14 cases with divergent transitions, we only report those with less than 5 divergent transitions
expect_equal(nrow(subset(bayes_r_raw %>% filter(!is.na(warnings)))), 14)
...
bayes_r <- bayes_r_raw %>%
  mutate(model = paste(group, "by", predictor),
         coefficient = case_when(
           model == "taste_group by subsistence" ~ NA,
           TRUE ~ coefficient
         ),
         lower_ci = case_when(
           model == "taste_group by subsistence" ~ NA,
