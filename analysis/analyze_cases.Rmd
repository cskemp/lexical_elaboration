...
create_table_plot <- function(concept, top_five_langs, top_five_fams, top_terms) {
  ...
  geom_rect(
    data = header1_data,
    aes(xmin = 0.4, xmax = 1, ymin = -Row - 0.4, ymax = -Row + 0.4),
    fill = "gray50", inherit.aes = FALSE
  ) +
  geom_text(aes(x = 0.7, label = table_data$Column1, fontface = fontface), 
            size = ifelse(table_data$Column1 %in% c("Top languages", "Top families"), 2.8, 2.7), 
            hjust = 0.5, 
            color = ifelse(table_data$Column1 %in% c("Top languages", "Top families"), "white", "black")) +
  geom_rect(
    data = header2_data,
    aes(xmin = 1.01, xmax = 1.4, ymin = -Row - 0.4, ymax = -Row + 0.4),
    fill = "gray50", inherit.aes = FALSE
  ) +
  geom_text(aes(x = 1.2, label = table_data$Column2), 
            size = ifelse(table_data$Column2 == "Top terms", 2.8, 2.7), 
