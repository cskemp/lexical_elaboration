...
This notebook contains analysis of existing claims (see the main text section called "Testing 163 proposed examples of lexical elaboration"). It generates Figure 1 in the main text and Figure S5 in the Supporting Material.
...
output_dir <- "../data/foranalyses"
write_claims <- function(data, path, filename) {
  data %>%
    mutate(word_list = sapply(word_list, paste, collapse = ", ")) %>%
    write_csv(file = file.path(path, filename))
}
write_claims(d_claims_distinct, output_dir, "d_claims_distinct.csv")
```
...
other_snow <- c("Plains Cree", "Lakota", "Japanese", "Scots", "Navajo", "Dakota", "Ahtena", "Mi'kmaq", "North Saami", "Dena'ina", "Tibetan", "Central Carrier", "Tamil", "Cebuano", "Amharic", "Sinhala", "Tibetan", "German", "Russian", "Swedish", "Thai", "Tagalog", "Tanacross")
...
plot_snow_zeta_half <- snowcases %>%
  ...
  xlim(-1, 4) +
  geom_dotplot(binaxis = "y", stackdir = "down", dotsize = 0.12, binwidth = 0.3) +
  geom_dotplot(data = snowcases_inuit, binaxis = "y", stackdir = "down", dotsize = 1, binwidth = 0.3, fill = "red", color = "red") +
  theme_classic() +
  geom_text_repel(
    data = snowcases_labeled,
    aes(x = 1, y = zeta, label = langname, color = tcolor),
    direction = "y", force = 1.7, 
    hjust = 0, nudge_x = 0.1, segment.color = "gray",
    box.padding = 0.08, 
    point.padding = 0.2, 
    size = 3.6
  ) +
  ...
  labs(x = "All languages", y = expression(L^{lang} * " for SNOW"), title = "") +
  theme_font
plot_snow_zeta_half
...
labelb <- c("SMELL, Yue Chinese", "DOCTOR, Hindi", "DANCE, Hindi",
             ...
             "ACORN, Yana",  "FRIEND, Russian", #"BODY PARTS, Aleut",
             "SNOW, Eskimo-Aleut", "WIND, Oceanic",
             "ICE, Yupik",  "TASTE, Japanese", "GLANCE, Hindi", #"SMELL, Thao",
             "JUDGE, Swahili", "SIGHT, Yana", #"ANT, Mizo", "TREE, Mi'kmaq",
            #"BIRD, Papua New Guinea", "GARDENING, Oceanic", "TOGETHERNESS, Iraqw",
            "STARS, Australian", "TIME OF DAY, Hausa", "FISH, Palauan", "RICE, Asian",
            "BIRD, Fore", "HORSE, Portuguese", "HONEY, Arabic", "FERN, Maori", "BEER, German")
...
default_plot_dot <- default_p %>%
  ...
  xlim(0.5, 1.7) +
  geom_dotplot(binaxis = "y", stackdir = "down", dotsize = 0.125, binwidth= 0.025)  +
  geom_dotplot(data = eskimosnow, binaxis = "y", stackdir = "down", dotsize = 0.04, binwidth= 0.3, fill="red", color="red")  +
  geom_text_repel(data = default_p_labeled, aes(x=1,y = rank, label = claim, color=dot_color), direction = "y", force = 1, hjust = 0,  nudge_x = 0.05, segment.color= "gray", box.padding = 0.08, size = 3.6) +
  scale_color_manual(values = c("black", "red")) +
  labs(x="All examples", y="rank", title = "") +
  theme_classic() +
  theme(legend.position="none") + 
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.line.x = element_blank()) +
...
compressed_plot_ids <- default_p_ids %>%
  ...
  geom_violinhalf(flip = TRUE, fill = NA) +
  geom_dotplot(binaxis = "y", stackdir = "down", dotsize = 0.3, binwidth= 0.025)  +
  ...
  geom_text_repel(data = default_p_ids_labeled, aes(x=chapter_name,y = rank, label = concept), direction = "y", force = 1, hjust = 0,  nudge_x = 0.05, segment.color= "gray", box.padding = 0.08, size = 3) +
  labs(x="Domains", y="rank", title = "") +
  ...
  theme_font + theme(axis.text.x = element_text(size = 10))

compressed_plot_ids

plot_width <- c(0.26,0.27,0.48) 
...
ggsave(filename = here::here("output", "figures", "main", "claims.pdf"), plot = combined_plots, device = "pdf", width = 14, height = 9)
