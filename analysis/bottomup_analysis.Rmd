...
This notebook contains two bottom-up analyses: a) exhaustive logistic regression analysis on a subset of 2246 terms with natural and cultural variables as predictors, and b) identification of nearest neighbour terms. It generates Figure 3 in the main text, and Figure S6 and Table S5 in the Supporting Material. Note that it takes several hours to generate results for exhaustive analyses.
...
Prepare a data frame: we first take out Wiktionary filtered forms. We then standardize all predictor variables except for subsistence strategy which is a categorical variable. Note that population size is first log-transformed and then standardized.
...
dstats <- d_long %>%
  ...
  anti_join(wikfilter, by = c("glottocode", "word"))
...
# convergence issues for 1 cases
withwarnings <- pvals %>% filter(!is.na(warnings_m1) | !is.na(warnings_m2))
expect_equal(nrow(withwarnings), 1)
...
# convergence issues for 233 cases
withwarnings <- pvals_rob %>% filter(!is.na(warnings_m1) | !is.na(warnings_m2))
expect_equal(nrow(withwarnings), 233)
...
make_plot <- function(pvals, predictorname, termlist, lowerbound, upperbound, manualbin, breaknumber) {
  ...
  combined <- pvals %>% 
    ...
    mutate(lnpval = case_when(
      p_val == 1 ~ abs(log(0.9999)),
      TRUE ~ lnpval
    ),
...
wndlist <- c("wind", "jungle", "fruit", "ice", "chimney", "bamboo", "crocodile")
