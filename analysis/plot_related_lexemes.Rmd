...
smell_plot <- smell_related_lexemes %>%
  ...
  annotate("text", x = 62, y = Inf, label = "r = 0.47***", size = 4, hjust = 1.2, vjust = 1.2)
show(smell_plot)
```
...
taste_plot <- taste_related_lexemes %>%
  ...
  annotate("text", x = 50, y = Inf, label = "r = 0.43***", size = 4, hjust = 1.2, vjust = 1.2)
show(taste_plot)
```
...
leave_one_out_correlation <- function(data, col_x, col_y) {
  results <- sapply(1:nrow(data), function(i) {
    ...
    cor(temp_data[[col_x]], temp_data[[col_y]], use = "complete.obs")
  })
