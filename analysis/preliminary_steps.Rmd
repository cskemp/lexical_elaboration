...
# notice that we do not filter out wiktionary filtered forms here because it will be filtered out in analyze_claims.Rmd and analyze_cases.Rmd later.

```

## Write data for running hierarchical model

Define a subset of words on which we run logistic regression. We keep words that appear in dictionaries of more than 30 languages, which results in 7973 terms in total.
...
if (filter_type == "original") {
  expect_equal(length(subset_lr), 7973)
} else {
  expect_equal(length(subset_lr), 7955)
}

```

Prepare a data frame for running hierarchical model to compute L^lang, L^fam, and L^dict scores:
...
wikfilter <- read_csv(here("data", "foranalyses", "wiktionary_filtered_combinations.csv"), show_col_types = FALSE) %>%
  select(glottocode, word) %>% unique() %>% rename(lang = glottocode)

d_wide <- d_long %>% 
  mutate(dict = id, lang = gcode_data, langname = langname_data, family = langfamily_data) %>% 
  select(dict, lang, langname, family, word, count) %>%
  # filter out wiktionary filtered forms
  anti_join(wikfilter, by = c("lang", "word")) %>%
...
bila <- read_csv(dict_path) %>% 
  ...
  distinct() %>%
  arrange(population)
expect_equal(length(unique(bila$glottocode)), 616)
...
expect_equal(length(unique(bila_filtered$glottocode)), 231)
...
Next we keep dictionaries that are relatively large in size for the set of languages defined above. We take the first quartile as a threshold for all token counts and keep dictionaries with token size above this threshold. This results 261 dictionaries in 226 languages.
...
if (filter_type == "original") {
  expect_equal(nrow(d_robust), 261)
  expect_equal(length(unique(d_robust$glottocode)), 226)
} else {
  expect_equal(nrow(d_robust), 266)
  expect_equal(length(unique(d_robust$glottocode)), 230)
}
...
d_long <- d_long %>% 
  ...
  select(id, langname_data, gcode_data, area_data, langfamily_data, population_data, subsistence_data, longitude_data, latitude_data, avgmonth_tmp_data, minmonth_tmp_data, maxmonth_pre_data, maxmonth_wnd_data, logpop_data, word, count, dictsize_data) %>%
  # filter out wiktionary filtered forms
  anti_join(wikfilter %>% rename(gcode_data = lang), by = c("gcode_data", "word")) %>%
