...
We ran `wiktionary_extract.ipynb` to identify cases where a form of word or morphological unit in a language coincides with a particular English word (e.g., Afrikaans *die*). Using the resulting data frame `../data/forpreprocessing/wiktionary_forms.tsv`, we then ran `wiktionary_filter.R` to filter all words. This filtering step is desirable to eliminate spurious noises, such as Afrikaans *die* (which refers to *the* article and hence have many counts). The filtered data frames are under the names `bila_long_nounverbadj_full.csv` and `bila_matrix_nounverbadj_full.csv` in the folder `../data/biladataset`.
